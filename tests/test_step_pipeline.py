...
_ENGINE_STEPS = {"filter", "download", "summarize", "crossdedupe", "coverage", "rate", "cluster", "select", "draft", "rewrite"}
# Steps that do NOT accept --engine
_NO_ENGINE_STEPS = {"start", "gather", "send"}
...
def test_run_all_steps_in_order(tmp_db, tmp_path, monkeypatch, sample_sources_yaml):
    ...
    from lib.steps.pipeline import cli as run_cli

    with patch("lib.steps.pipeline._STEP_CLIS", mock_clis), \
...
def test_run_from_step(tmp_db, tmp_path, monkeypatch):
    ...
    expected_skipped = ["start", "gather", "filter", "download", "summarize", "rate"]
