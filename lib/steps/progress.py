"""newsagent:progress — show workflow progress for a session.

CLI entry: python -m lib.steps.progress [--db PATH] [--session SID]
"""
...
def cli(db_path: str, session_id: str | None) -> None:
    ...
    if session_id is None:
        ...
        if not recents:
            click.echo("No sessions found. Run /newsagent:start to create one.")
            return
