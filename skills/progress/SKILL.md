...
```bash
python -m lib.steps.progress [--db newsletter_agent.db] [--session SID]
```
...
- After `/newsagent:start` to confirm setup.
- During a run to see what's running / errored.
- Before `/newsagent:recover` to decide what to re-execute.
